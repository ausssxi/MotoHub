...
async def collect_bds_models():
    async with async_playwright() as p:
        print("BDSモデルコレクターを起動しています...")
        ...
        try:
            for m in maker_list_raw:
                ...
                # メーカーレコードの取得または作成
                m_record = db.query(Manufacturer).filter(Manufacturer.name.like(f"%{m['name']}%")).first()
                if not m_record:
                ...
                try:
                    await page.goto(m_url, wait_until="networkidle", timeout=60000)
                    ...
                    for block in model_blocks:
                        ...
                        if not existing_idnt:
                            ...
                            print(f"  [新登録] {model_name} (ID: {identifier_val})")
                    
                    db.commit()
                except Exception as e:
                    print(f"  エラー ({m['name']}): {e}")
                    db.rollback()

            print("\nBDS車種マスタの全件同期が完了しました。")
        except Exception as e:
            print(f"致命的エラー: {e}")
        finally:
            db.close()
...
if __name__ == "__main__":
    asyncio.run(collect_bds_models())