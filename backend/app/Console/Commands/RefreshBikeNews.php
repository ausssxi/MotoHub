...
final class RefreshBikeNews extends Command
{
    protected $signature = 'news:refresh
        {--limit=2000 : 1回の実行で取得する最大モデル数}
        {--all : キャッシュ有無を問わず取得する（デフォルトは未保持＝失効分のみ）}';

    protected $description = '在庫あり車種の車種別ニュース(Google News RSS)をキャッシュへ事前取得（render pathから分離・日次ローテーション）';

    public function handle(BikeNewsService $news): int
    {
        ...
        foreach ($models as $model) {
            ...
            if (! $all && Cache::has(BikeNewsService::cacheKey($model->id))) {
                $skipped++;

                continue;
            }

            $news->refreshForModel($model);
            $done++;
        }
