...
final class GenerateMonthlyMarketReport extends Command
{
    ...
    private function callClaudeApi(
        ...
        $dispText = collect($displacementData)->map(fn ($d) => "{$d['label']}: {$d['sold_count']}台（前月比 {$d['change_rate']}%）/ 平均".number_format($d['avg_price']).'円'
        )->implode("\n");

        $makerText = collect($makerData)->map(fn ($m, $i) => ($i + 1)."位 {$m['name']}: {$m['sold_count']}台"
        )->implode("\n");
        ...
        $priceText = collect($priceBands)->map(fn ($p) => "{$p['label']}: {$p['count']}台"
        )->implode("\n");
