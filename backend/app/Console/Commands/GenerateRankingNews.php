...
final class GenerateRankingNews extends Command
{
    ...
    private function generateComment(
        ...
        if ($currentPrice && $prevPrice) {
            ...
            if ($diff > 0) {
                $priceUp = round($diff, 1);
            }
            if ($diff < 0) {
                $priceDown = round(abs($diff), 1);
            }
        }
    ...
    private function buildContent(
        ...
        foreach ($rankings->values() as $i => $row) {
            ...
            // 自動コメント
            // 前期間データが取得できなかった場合（月次・週次は売却済みListingが約2週間で消えるため
            // 前期間が丸ごと空になりうる）は比較材料が無い。この状態を「初のランクイン」等と
            // 断定するのは事実に反するため、コメント自体を出さない（前期間データがある場合のみ生成）。
            $prevPrice = isset($prevAvgPrices[$row->bike_model_id]) ? (float) $prevAvgPrices[$row->bike_model_id] : null;
            $comment = ! empty($prevRanks)
                ? $this->generateComment($rank, $prevRank, $row->sold_count, $currentPrice, $prevPrice)
                : null;
