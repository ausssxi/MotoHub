...
final class GsiGeocodingService
{
    ...
    /**
     * 市区町村の補正表（CITY_CORRECTIONS / STREET_WARD_REPLACEMENTS）と適用ロジックは
     * App\Support\MunicipalityCorrections へ切り出した。座標取得だけでなく city カラム自体の
     * 是正（addresses:fix-city）や住所パース（AddressParser）からも同じ知識を使うため。
     *
     * ここではイベント種別に対する「このサービスのログ文言」だけを持つ。表の中身・適用順序・
     * 判定条件は切り出し前とまったく同じなので、ジオコーディングの結果は変わらない。
     *
     * @var array<string, string>
     */
    private const CORRECTION_LOG_MESSAGES = [
        MunicipalityCorrections::EVENT_SPLIT_WARD_MATCHED => 'GSI geocoding: 分割区を street 先頭一致で振り分け',
        MunicipalityCorrections::EVENT_SPLIT_WARD_DEFAULT => 'GSI geocoding: 分割区を既定へ振り分け（例外町名の追加候補）',
        MunicipalityCorrections::EVENT_CITY_CORRECTION => 'GSI geocoding: 廃止・改称された市区町村を補正',
        MunicipalityCorrections::EVENT_STREET_WARD_REPLACEMENT => 'GSI geocoding: 廃止された行政区を street で置換',
    ];
     ...
     *
     * 判定・置換そのものは MunicipalityCorrections::apply() が行い、ここでは戻ってきた
     * イベント列を従来と同じ文言・同じ context・同じ順序で Log::info するだけ。
     *
    ...
    {
        [$city, $address, $events] = MunicipalityCorrections::apply($prefecture, $city, $address);

        foreach ($events as $event) {
            $message = self::CORRECTION_LOG_MESSAGES[$event['type']] ?? null;
            if ($message !== null) {
