...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                 ...
                 */
                /*
                 * UGC（ユーザー投稿・記事画像）だけを含める。消えたら二度と戻らないため。
                 * スクレイプ由来の画像（listings/models/manufacturers/shops/ogp/x-images 等）は
                 * 再取得可能かつ巨大なので含めない（include に列挙しない＝対象外）。
                 * .env は APIキー群を含むが、下記の password によるzip暗号化を必須とする。
                 */
                'include' => [
                    storage_path('app/public/blog'),          // ブログ記事画像（writer投稿）
                    storage_path('app/public/shop-user'),     // 承認済みユーザー投稿の店舗画像
                    storage_path('app/private/shop-submissions'), // 承認待ちの投稿画像
                    storage_path('app/private/garage'),       // ユーザーのガレージ画像
                    base_path('.env'),                        // ※ 暗号化前提
                ],
        ...
        'destination' => [
             ...
             */
            /*
             * R2 の認証情報（R2_ENDPOINT）があれば r2、無ければ local に保存する。
             * ローカル/非本番は R2 無しで検証が回る。BACKUP_DISK で明示上書きも可。
             */
            'disks' => [
                env('BACKUP_DISK', env('R2_ENDPOINT') ? 'r2' : 'local'),
            ],
     ...
     */
    'notifications' => [
        // 失敗・不健全だけメール通知。成功系はオフ（ノイズを増やさない）。
        'notifications' => [
        ...
        'mail' => [
            'to' => env('BACKUP_NOTIFICATION_EMAIL', env('CONTACT_ADMIN_EMAIL', 'info@motohub.jp')),
    ...
    'monitor_backups' => [
        [
            'name' => env('APP_NAME', 'laravel-backup'),
            'disks' => [env('BACKUP_DISK', env('R2_ENDPOINT') ? 'r2' : 'local')],
            'health_checks' => [
                MaximumAgeInDays::class => 1,        // 最新が1日より古ければ不健全→通知
                MaximumStorageInMegabytes::class => 9000, // R2無料枠10GBの内側
            ],
        ],
    ...
    'cleanup' => [
        ...
        'default_strategy' => [
             ...
             */
            'delete_oldest_backups_when_using_more_megabytes_than' => 9000, // R2無料枠10GBの内側で自動トリム
        ],
