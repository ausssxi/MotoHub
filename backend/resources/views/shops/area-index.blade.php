<x-layout>
    <x-slot:title>エリアからバイクショップを探す | MotoHub</x-slot:title>
    <x-slot:metaDescription>全国{{ number_format($totalCount) }}店のバイクショップを都道府県別に一覧表示。お近くのバイク販売店・整備工場を探せます。</x-slot:metaDescription>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8 mb-8">
                ...
                <p class="text-sm text-gray-500">
                    全国<span class="font-bold text-blue-600">{{ number_format($totalCount) }}</span>店のバイクショップ・販売店を都道府県別に掲載しています。
                </p>
            </div>
            ...
            <section class="mb-8">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    ...
                    <a href="{{ $count > 0 ? route('shops.area.prefecture', $pref) : '#' }}"
                       class="bg-white rounded-xl border border-gray-100 p-4 hover:border-blue-200 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 block {{ $count === 0 ? 'opacity-50 pointer-events-none' : '' }}">
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-black text-gray-800">{{ $pref }}</span>
                            <span class="text-xs font-bold {{ $count > 0 ? 'text-blue-600' : 'text-gray-300' }}">
            ...
            <div class="text-center mt-8">
                <a href="{{ route('shops.map') }}"
                   class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm py-3 px-8 rounded-xl transition-colors shadow-sm">
            ...
            <a href="{{ route('shops.submit.create') }}"
               class="mt-8 flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-2xl p-5 hover:shadow-md transition-shadow">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-black text-gray-800">掲載されていないバイクショップをご存知ですか？</p>
