<x-layout>
    ...
    <x-slot:scripts>
        ...
        <script src="{{ asset('js/riders/map.js') }}?v={{ asset_buster(public_path('js/riders/map.js')) }}"></script>
    ...
    <div class="relative w-full">
      <div class="relative w-full" id="map-stage">
        ...
        <div id="layer-chips" class="absolute top-14 left-3 right-3 z-40 flex flex-nowrap overflow-x-auto scrollbar-hide gap-1.5 pb-1"
             x-data="{
                 shop: true, parking: true, gas: false, cvs: false, michi: false, carwash: false, rentalgarage: false, rentalbike: false, blog: false, saved_spots: {{ auth()->check() ? 'true' : 'false' }},
                 notify() {
                     let l = {shop: this.shop, parking: this.parking, gas_station: this.gas, convenience_store: this.cvs, michi_no_eki: this.michi, car_wash: this.carwash, rental_garage: this.rentalgarage, rental_bike: this.rentalbike, blog: this.blog, saved_spots: this.saved_spots};
             ...
             x-init="
                 ...
                 if (new URLSearchParams(window.location.search).get('layer') === 'rental_bike') { rentalbike = true; }
                 window.ridersMapLayers = {shop: true, parking: true, gas_station: false, convenience_store: false, michi_no_eki: false, car_wash: false, rental_garage: rentalgarage, rental_bike: rentalbike, blog: false, saved_spots: {{ auth()->check() ? 'true' : 'false' }}};
             ">
            ...
            <button type="button" @click="rentalbike = !rentalbike; notify()"
                    class="layer-btn px-2.5 py-1.5 rounded-lg text-[11px] font-bold flex items-center gap-1"
                    :style="rentalbike ? 'background:#ec4899;color:#fff;border:2px solid #ec4899' : 'background:#fff;color:#4b5563;border:2px solid #e5e7eb'">
                <span class="text-sm leading-none">&#x1F511;</span>
                レンタルバイク
            </button>
        ...
        <div id="map-legend" class="absolute bottom-16 right-3 z-40" x-data="{ open: false }">
            ...
            <div x-show="open" x-cloak x-transition
                ...
                {{-- ※ 色は public/js/riders/map.js の layerConfig(rental_bike=#ec4899) と揃えること。arbitrary クラスを増やさぬよう border-color はインライン --}}
                <p class="text-[10px] text-gray-400 mb-1 pt-1.5 border-t border-gray-100">レンタルバイク</p>
                <div class="flex items-center gap-2"><span class="inline-flex w-5 h-5 rounded-full bg-white border-[3px] items-center justify-center text-[9px]" style="border-color:#ec4899">&#x1F511;</span> レンタルバイク店舗</div>
            </div>
        </div>
      </div>{{-- /#map-stage --}}
