<x-layout>
    <x-slot:title>バイク盗難データ 都道府県別ランキング・推移と盗難対策｜MotoHub</x-slot:title>
    <x-slot:metaDescription>警察庁の犯罪統計（オートバイ盗）をもとに、バイク盗難の都道府県別 認知件数ランキング・検挙率・経年推移と、効果的な盗難対策をMotoHubがまとめました。</x-slot:metaDescription>
    ...
    <div class="max-w-3xl mx-auto px-4 py-8 sm:py-12">
        <header class="mb-8">
            <h1 class="text-2xl sm:text-3xl font-black text-gray-900 leading-tight">バイク盗難データ｜都道府県別ランキングと対策</h1>
            <p class="text-sm text-gray-500 mt-3 leading-relaxed">警察庁の犯罪統計（オートバイ盗）をもとに、都道府県別の認知件数・検挙率・推移を淡々とまとめています。数字はそのまま掲載し、過度に不安を煽らないことを方針としています。</p>
            @if($hasData)
            <p class="text-[11px] text-gray-400 font-bold mt-2">対象年 {{ $source['data_year'] }}年・最終確認 {{ $source['checked_at'] }}</p>
            @endif
        </header>

        @if($hasData)
            {{-- 全国推移 --}}
            @if(count($nationalSeries) >= 2)
            <section class="mb-8 bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
                <h2 class="text-base font-black text-gray-900 mb-3">全国のオートバイ盗 認知件数の推移（{{ $nationalSeries[0]['year'] }}〜{{ end($nationalSeries)['year'] }}年）</h2>
                @include('bikes.partials.sparkline', ['points' => array_column($nationalSeries, 'recognized'), 'w' => 640, 'h' => 120, 'color' => '#dc2626', 'label' => '全国のオートバイ盗認知件数の推移'])
                <div class="flex justify-between text-[11px] text-gray-400 font-bold mt-1">
                    <span>{{ $nationalSeries[0]['year'] }}年 {{ number_format($nationalSeries[0]['recognized']) }}件</span>
                    <span>{{ end($nationalSeries)['year'] }}年 {{ number_format(end($nationalSeries)['recognized']) }}件</span>
                </div>
            </section>
            @endif

            {{-- 都道府県別ランキング --}}
            <section class="mb-8 bg-white rounded-3xl shadow-sm border border-gray-100 p-4 sm:p-6">
                <h2 class="text-base font-black text-gray-900 mb-4">都道府県別 オートバイ盗ランキング（{{ $source['data_year'] }}年・認知件数）</h2>
                <div class="overflow-hidden rounded-2xl border border-gray-100">
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 text-gray-500 text-xs">
                                <th class="text-left font-bold px-3 py-2">順位</th>
                                <th class="text-left font-bold px-3 py-2">都道府県</th>
                                <th class="text-right font-bold px-3 py-2">認知件数</th>
                                <th class="text-right font-bold px-3 py-2">検挙率</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @foreach($ranking as $r)
                            <tr>
                                <td class="px-3 py-2 font-black text-gray-400">{{ $r['rank'] }}</td>
                                <td class="px-3 py-2 font-bold"><a href="{{ route('bikes.area_index', $r['prefecture']) }}" class="text-gray-800 hover:text-red-600 hover:underline">{{ $r['prefecture'] }}</a></td>
                                <td class="px-3 py-2 text-right font-black text-red-600">{{ number_format($r['recognized']) }}</td>
                                <td class="px-3 py-2 text-right text-gray-600">{{ $r['clearance_rate'] !== null ? $r['clearance_rate'].'%' : '-' }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <p class="text-[10px] text-gray-400 mt-3">
                    出典: <a href="{{ $source['url'] }}" target="_blank" rel="nofollow noopener" class="underline hover:text-gray-600">{{ $source['label'] }}</a>（最終確認: {{ $source['checked_at'] }}）。検挙率・順位は認知・検挙件数から算出。
                </p>
            </section>
        @else
            <section class="mb-8 bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
                <p class="text-sm text-gray-500">都道府県別の統計データは準備中です。公開までしばらくお待ちください。</p>
            </section>
        @endif
        ...
        <section class="mb-8 bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
            ...
            <ul class="space-y-2 text-sm text-gray-600 leading-relaxed list-disc pl-5">
                ...
                <li><span class="font-bold text-gray-800">車体カバー</span>：車種を特定されにくくし、物色対象になりにくくする。</li>
