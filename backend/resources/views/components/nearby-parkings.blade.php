...
<section class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
    ...
    <div class="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
        ...
        <a href="{{ route('parking.show', $p->id) }}" class="snap-start shrink-0 w-56 sm:w-64 group bg-green-50/50 rounded-xl p-4 border border-green-100 hover:border-green-300 hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 block">
            <div class="flex items-start justify-between mb-2">
                <h4 class="text-xs font-black text-gray-800 group-hover:text-green-700 transition-colors line-clamp-2 flex-1 mr-2">{{ $p->name }}</h4>
                @if($p->avg_rating > 0)
