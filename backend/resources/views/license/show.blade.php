<x-layout>
    ...
    <div class="bg-gray-50 min-h-screen">
        ...
        <div class="max-w-3xl mx-auto px-4 py-10 space-y-10">
            ...
            {{-- 合宿免許アフィリバナー（★url と banner_url が揃うときのみ・PR表記付き。未設定なら枠ごと出さない） --}}
            @php
            ...
            @if(!empty($licAff['url']) && !empty($licAff['banner_url']) && request()->route('class') !== 'gentsuki')
                <section>
                    <div class="bg-white rounded-2xl border border-slate-200 p-6 text-center">
                        ...
                        @if(!empty($licAff['provider']))
                            <p class="text-[10px] font-bold text-slate-400 mt-3">提供: {{ $licAff['provider'] }}・PR</p>
                        @endif
                        @if(!empty($licAff['imp_url']))
                            <img src="{{ $licAff['imp_url'] }}" width="1" height="1" alt="" style="border:0;position:absolute;left:-9999px;" aria-hidden="true">
                        @endif
                    </div>
                </section>
            @endif
