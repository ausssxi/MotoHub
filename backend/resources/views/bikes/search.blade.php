<x-layout>
    ...
    <x-slot:scripts>
        ...
        <script src="{{ asset('js/search/infinite-scroll.js') }}?v={{ asset_buster(public_path('js/search/infinite-scroll.js')) }}" defer></script>
        @guest
        <script>
                    ...
                    <form action="{{ route('bikes.search') }}" method="GET" id="filter-form" class="p-6 space-y-8 overflow-y-auto">
                        ...
                        <div class="filter-group">
                            ...
                            @php
                                ...
                                // 「都/道/府/県」を機械的に剥がす方式は「京都府」→「京」(都も府も除去)のように
                                // 二重剥がれを起こすため廃止。47都道府県の短縮名↔正式名 対応表で突合・合算する。
                                $shortToFull = [
                                    '北海道' => '北海道',
                                    '青森' => '青森県', '岩手' => '岩手県', '宮城' => '宮城県',
                                    '秋田' => '秋田県', '山形' => '山形県', '福島' => '福島県',
                                    '茨城' => '茨城県', '栃木' => '栃木県', '群馬' => '群馬県',
                                    '埼玉' => '埼玉県', '千葉' => '千葉県', '東京' => '東京都',
                                    '神奈川' => '神奈川県',
                                    ...
                                    '広島' => '広島県', '山口' => '山口県',
                                    '徳島' => '徳島県', '香川' => '香川県', '愛媛' => '愛媛県',
                                    '高知' => '高知県',
                                    '福岡' => '福岡県', '佐賀' => '佐賀県', '長崎' => '長崎県',
                                    '熊本' => '熊本県', '大分' => '大分県', '宮崎' => '宮崎県',
                                    '鹿児島' => '鹿児島県', '沖縄' => '沖縄県',
                                ];
                                // 正式名 → 短縮名 の逆引き（ファセットキーがフルネームの場合に短縮名へ寄せる）
                                $fullToShort = array_flip($shortToFull);

                                $prefCounts = [];
                                foreach (($facets['prefecture'] ?? []) as $rawPref => $cnt) {
                                    if (isset($shortToFull[$rawPref])) {
                                        $short = $rawPref;                 // 既に短縮名（京都・大阪・北海道等）
                                    } elseif (isset($fullToShort[$rawPref])) {
                                        $short = $fullToShort[$rawPref];   // 正式名（京都府等）→短縮名
                                    } else {
                                        $short = $rawPref;                 // 対応表外はそのまま
                                    }
